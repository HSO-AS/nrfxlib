...
impl TcpSocket {
    ...
    pub fn connect(&self, hostname: &str, port: u16) -> Result<(), Error> {
        ...
        let mut result = sys::NRF_EAI_AGAIN as i32;
        while result == sys::NRF_EAI_AGAIN as i32 {
            ...
            debug!("nrf_getaddrinfo: {}", result);
            unsafe { sys::nrf_modem_os_busywait(1000_000) };
        }
        if (result == 0) && (!output_ptr.is_null()) {
            ...
            debug!("loop: {}, {}", result, output_ptr.is_null());
        }
        if result != 0 {
